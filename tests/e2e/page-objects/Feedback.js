...
export default class Feedback {
  constructor() {
    this.link = {
      documentation: Selector('a').withText('Managing customizations using AJAX (CRUD operations)')
    };
    this.input = {
    ...
    this.button = {
      ...
      refresh: Selector('.btn.btn-primary.ml-3').withText('Refresh content from the server')
    };
    this.radio = {
      radioExcellent: Selector('label').withText('Excellent'),
      radioMeh: Selector('.row > .mt-3:nth-of-type(2) label:nth-of-type(2)').withText('Meh')
    };
    this.table = {
